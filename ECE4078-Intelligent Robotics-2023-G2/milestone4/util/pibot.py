...
class Alphabot:
    ...
    def set_velocity(self, command, tick=10, turning_tick=5, time=0):
        ...
        r_vel = 0.99* command[0] * tick + command[1] * turning_tick
        ...
        if time == 0:
            ...
            requests.get(
                "http://" + self.ip + ":" + str(self.port) + "/robot/set/velocity?value=" + str(l_vel) + "," + str(
                    r_vel)
                + "&time=" + str(time))
        return l_vel, r_vel
    ...
    def get_image(self):
        ...
        requests.exceptions.ConnectTimeout, requests.exceptions.ConnectionError, requests.exceptions.ReadTimeout) as e:
