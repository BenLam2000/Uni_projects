...
def drive_to_point(waypoint):
    ...
    robot_pose[2] = (np.arctan2(y_diff, x_diff) + np.pi) % (
                2 * np.pi) - np.pi  # limits range of angle to the range [0,2pi]
...
def detect():
    ...
    bboxes, yolo_vis = detector.detect_single_image(img=yolo_input_img)
    new_fruit_found = False
    if bboxes:  # if fruit is detected
        for bbox in bboxes:
            ...
            if label not in search_list:
                print(f"New fruit '{label}' detected")
                new_fruit_found = True
                target_pose = estimate_pose(camera_matrix, bbox, relative_pose_only=False)
    ...
    generate_move_path(target_fruits, fruit)
    return new_fruit_found


def generate_move_path(target_fruits, fruit):
    ...
    for waypoint in path:
        ...
        new_fruit_found = drive_to_point(waypoint)
        if new_fruit_found:
            break
...
if __name__ == "__main__":
    shutil.copy("M4_true_map.txt", "M4_true_map_Copy.txt")
