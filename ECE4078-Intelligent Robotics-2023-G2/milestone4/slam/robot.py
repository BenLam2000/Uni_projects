...
class Robot:
    ...
    def derivative_drive(self, drive_meas):  # computes A
        ...
        # add your codes here to compute DFx using lin_vel, ang_vel, dt, and th
        DFx[0,1] = 0
        DFx[1,0] = 0
        DFx[2,0] = 0
        DFx[2,1] = 0
        ...
        if ang_vel == 0:
            ...
            DFx[0,2] = (lin_vel/ang_vel) * (np.cos(th) * (np.sin(ang_vel*dt) - 1) - np.sin(th) * np.sin(ang_vel*dt))
            DFx[1,2] = (lin_vel/ang_vel) * (np.sin(th) * (np.cos(ang_vel*dt) - 1) + np.cos(th) * np.sin(ang_vel*dt))

        return DFx

    def derivative_measure(self, markers, idx_list):  # computes C
    ...
    def covariance_drive(self, drive_meas):  # computes sigma_Q
        ...
        if ang_vel == 0:
            ...
            Jac2[0,1] = (-1/(ang_vel**2)) * (lin_vel) * (-np.sin(th) + np.sin(th2))
            Jac2[1,0] = (1/ang_vel) * (np.cos(th) - np.cos(th2))
            Jac2[1,1] = (-1/(ang_vel**2)) * (lin_vel) * (np.cos(th) - np.cos(th2))
            Jac2[2,1] = dt

        # Derivative of x,y,theta w.r.t. left_speed, right_speed (chain rule)
        Jac = Jac2 @ Jac1

        # Compute covariance
        cov = np.diag((drive_meas.left_cov, drive_meas.right_cov))  # uncertainty(covariance) in left and right wheel
        cov = Jac @ cov @ Jac.T  # computes how the uncertainty in left and right wheel propagates to the full motion model (x,y,theta)
