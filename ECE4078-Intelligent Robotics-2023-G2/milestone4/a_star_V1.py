...
def a_star(truemap_file, end, start=(0,0)):
    ...
    start1 = start[1]*10
    end1 = end[1]*10
    start_tuple = (8 - start1/2, 8 + start0/2)             # For Level 2: hardcoded starting point (origin)
    end_tuple = (8 - end1/2, 8 + end0/2)
    for pose in js.keys():
        x_coor = js[pose]['x']
        y_coor = js[pose]['y']
        obstacle_column_index = 8 + x_coor/0.2
        obstacle_row_index = 8 - y_coor/0.2
    ...
    # Open the txt file to be overwritten
...
if __name__ == "__main__":
    path = a_star('./test_data/TRUEMAP.txt')
    print(path)