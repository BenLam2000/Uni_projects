...
class AutoFruitSearch:
    def __init__(self, scale, baseline, baseline_left=None, baseline_right=None):
    ...
    def drive_to_point(self, waypoint, robot_pose):
        ...
        # turn_time = 0.0
        # if dir == 1:
        turn_time = self.baseline*abs(desired_heading)/(2*self.scale*turn_ticks)  # replace with your calculation
        # elif dir == -1:
        #     turn_time = self.baseline*abs(desired_heading)/(2*self.scale*turn_ticks)  # replace with your calculation
        ...
        drive_time = distance_to_goal/(drive_ticks*self.scale)
    ...
    def destination_reached(waypoint, robot_pose):
    ...
    def automate_turn(self,robot_pose):
        turn_ticks = 15

        # turn 30 degrees
        turn_time = self.baseline * (np.pi/6) / (2 * self.scale * turn_ticks)

        print("Turning for {:.2f} seconds".format(turn_time))
        self.pibot.set_velocity([0, 1], turning_tick=turn_ticks, time=turn_time)
    ...
    def generate_move_path(self, robot_pose):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--map", type=str, default='M4_true_map_Copy.txt')
    ...
    if level == 1:
        ...
        while True:
            ...
            new_pose = drive_to_point(waypoint, robot_pose)
            reached = destination_reached(waypoint, new_pose)
            if reached:
                robot_pose = new_pose
                print("Finished driving to waypoint: {}; New robot pose: {}".format(waypoint,robot_pose))
            else :
                new_pose = drive_to_point(waypoint, new_pose)
                robot_pose = new_pose
            # resume slam and do 360 to update robot pose
