...
import generateGroundTruthRobotPose
...
class Operate:
    def __init__(self, args):
        ...
        self.drive_ticks = 25
        # self.turn_ticks = 15
        self.turn_left_ticks = 10
        self.turn_right_ticks = 10
        ...
        self.move_mode = 1  # 1 - big steps, 2- small steps
        self.measure_mode = 1  # 1 - xy, 2 - theta
        ...
        self.grad_tolerance = 1.0
        self.vec_angle_tolerance = 15
        self.marker_taglist = []
        self.marker_dist = []
        self.marker_rel = []
    ...
    def control(self):       
        if args.play_data:
            ...
            if self.command['motion'] == [1, 0]:  # forward
                lv, rv = self.pibot.set_velocity(
                    [0.98, 0], tick=self.drive_ticks)
            elif self.command['motion'] == [-1, 0]:  # back
                lv, rv = self.pibot.set_velocity(
                    [-0.88, 0], tick=self.drive_ticks)
            elif self.command['motion'] == [0, 1]:  # turn left
                lv, rv = self.pibot.set_velocity(
                    [0, 1.2], turning_tick=self.turn_left_ticks, tick=self.drive_ticks)
    ...
    def switch_move_mode(self):
        if self.command['switch_move_mode']:
            if self.move_mode == 1:
                self.move_mode = 2
            ...
            self.command['switch_move_mode'] = False
    ...
    def plot_new_map(self, map_, title_, map2_=''):
        aruco_x_list, aruco_y_list, aruco_num, aruco_x_2, aruco_y_2, aruco_num2, fruit_x_list, fruit_y_list, fruit_list = \
            generateGroundTruthRobotPose.parse_groundtruth(map_, map2_)
        generateGroundTruthRobotPose.plot_map(aruco_x_list, aruco_y_list, aruco_num, title_, aruco_x_2, aruco_y_2,
                                              aruco_num2, fruit_x_list, fruit_y_list, fruit_list)
    ...
    def measure_marker(self):
        if self.command['measure_marker']:
            ...
            if self.start_360_count == 360//self.turn360_angle + 1:
                ...
                self.plot_new_map('TRUEMAP.txt', 'Initial 4 Measured Markers (x,y)')
    ...
    def gradient_diff(self, x1, y1, x2, y2, x3, y3):
        grad1 = self.gradient(x1, y1, x2, y2)
        grad2 = self.gradient(x2, y2, x3, y3)
        return grad2 - grad1

    # get angle between vectors from 1 to 2 and 1 to 3
    @staticmethod
    def calc_vec_angle(x1, y1, x2, y2, x3, y3):
        pt1 = np.array([x1, y1])
        pt2 = np.array([x2, y2])
        pt3 = np.array([x3, y3])
        ...
        dot_product = np.dot(vec_12, vec_13)

        mag_12 = np.linalg.norm(vec_12)
        mag_13 = np.linalg.norm(vec_13)

        angle_rad = math.acos(dot_product/(mag_12*mag_13))
        angle_deg = math.degrees(angle_rad)

        return angle_deg

    def turn360_measure(self, canvas, waypoint=(0,0)):
        self.marker_dist = []
        self.marker_taglist = []
        self.marker_rel = []
        ...
        is_collinear = False
        vec_angle = abs(self.calc_vec_angle(sorted_markers[chosen_indices[0]][0], sorted_markers[chosen_indices[0]][1],
                                            sorted_markers[chosen_indices[1]][0], sorted_markers[chosen_indices[1]][1],
                                            sorted_markers[chosen_indices[2]][0], sorted_markers[chosen_indices[2]][1]))
        print(f"Vector angle: {vec_angle}")
        while vec_angle < self.vec_angle_tolerance or (180-vec_angle) < self.vec_angle_tolerance:  # gradient is too close
            is_collinear = True
            i += 1
            if i >= len(sorted_taglist):
                print("no extra markers to fix collinear error! Using waypoint directly")
                is_collinear = True

            chosen_indices[2] = i
            print(f"new taglist used: {sorted_taglist[i]}")
            print(f"new marker used: {sorted_markers[i]}")
            print(f"new dist used: {sorted_dist[i]}")
            vec_angle = abs(
                self.calc_vec_angle(sorted_markers[chosen_indices[0]][0], sorted_markers[chosen_indices[0]][1],
                                    sorted_markers[chosen_indices[1]][0], sorted_markers[chosen_indices[1]][1],
                                    sorted_markers[chosen_indices[2]][0], sorted_markers[chosen_indices[2]][1]))
            print(f"New vector angle: {vec_angle}")

            if not (vec_angle < self.vec_angle_tolerance or (180-vec_angle) < self.vec_angle_tolerance):
                is_collinear = False

        robot_x, robot_y = 0, 0
        if not is_collinear:
            robot_x, robot_y = self.trilaterate(sorted_markers[chosen_indices[0]][0], sorted_markers[chosen_indices[0]][1], sorted_dist[chosen_indices[0]],
                                                sorted_markers[chosen_indices[1]][0], sorted_markers[chosen_indices[1]][1], sorted_dist[chosen_indices[1]],
                                                sorted_markers[chosen_indices[2]][0], sorted_markers[chosen_indices[2]][1], sorted_dist[chosen_indices[2]])

            self.ekf.robot.state[0, 0] = robot_x
            self.ekf.robot.state[1, 0] = robot_y
        else:
            self.ekf.robot.state[0, 0] = waypoint[0]
            self.ekf.robot.state[1, 0] = waypoint[1]
    ...
    def turn360_measure_M3(self, canvas):
        if self.command['measure_marker'] and self.measure_mode == 1:
            self.marker_dist = []
            self.marker_taglist = []
            self.marker_rel = []
    ...
    # align robot theta with marker
    def align_theta_M3(self):
        if self.command['measure_marker'] and self.measure_mode == 2:
            print("Aligning robot theta")

            self.take_pic()  # update pibot image
            # get aruco detections
            lms, self.aruco_img = self.aruco_det.detect_marker_positions(self.img)
            lms = [lm for lm in lms if lm.tag <= 10]

            if len(lms) > 0:
                chosen_lm = random.choice(lms)
                chosen_tag = chosen_lm.tag
                chosen_marker_index = self.ekf.taglist.index(chosen_tag)
                chosen_marker = self.ekf.markers[:, chosen_marker_index]
                chosen_rel = chosen_lm.position
                print(f"Chosen Tag: {chosen_tag}")
                print(f"Chosen Marker Actual: {chosen_marker}")
                print(f"Chosen Marker Rel.: {chosen_rel}")
                ...
                print("No markers found! Please try again!")

            print(
                f"Robot State from SLAM: [{self.ekf.robot.state[0, 0]},{self.ekf.robot.state[1, 0]},{np.rad2deg(self.ekf.robot.state[2, 0])}]")

            # # switch back to big steps mode
            # self.move_mode = 1
            self.measure_mode = 1
    ...
    def run_fruit_search(self, canvas):
        ...
        for fruit in target_fruits:
            ...
            for waypoint in path:
                ...
                self.turn360_measure(canvas, waypoint)
    ...
    def update_keyboard(self):
        for event in pygame.event.get():
            ...
            # run target pose est to get targets.txt
                ...
                self.command['switch_move_mode'] = True
            # quit
...
if __name__ == "__main__":
    ...
    while True:
        if operate.mode == 1:  # initial 360
            ...
            # set default to move big steps first
            operate.move_mode = 1
            operate.measure_mode = 1

            while operate.mode == 3:
                operate.update_keyboard()
                operate.switch_move_mode()
                operate.take_pic()
                if operate.move_mode == 1:  # move big steps
                    operate.control()
                elif operate.move_mode == 2:  # move small steps
                    operate.move_small_step()
                operate.turn360_measure_M3(canvas)
                operate.align_theta_M3()
                operate.update_slam()
                operate.record_data()
                operate.save_image()
            ...
            operate.plot_new_map('TRUEMAP.txt', 'ARUCO + FRUITS', 'TRUEMAP_ALIGNED.txt')
            ...
            robot_x, robot_y = operate.trilaterate( -0.047139,  0.75826, 0.4240, 0.39882,  0.4205, 0.6283, 1.1493,   1.2469,  0.8821)
            print(f"x:{robot_x}  y:{robot_y}")
            while True:
                pass

        elif operate.mode == 9:
            operate.fruit_search.turn_combine(-165)
            while True:
                pass

        elif operate.mode == 10:
            vec_angle = operate.calc_vec_angle(0.47666, 0.49716, 1.3628, 1.0829,  0.5264, 1.1232)
            print(vec_angle)
            input("")
