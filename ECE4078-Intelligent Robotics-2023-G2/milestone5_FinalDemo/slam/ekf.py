...
class EKF:
    ...
    def __init__(self, robot):
        ...
        # State components
        self.robot = robot
        self.mode = 1  # 1-normal M2 ; 2-TRUEMAP
        self.markers = np.zeros((2,0))
        self.taglist = []
        self.measured_markers = []
        self.measured_taglist = []
    ...
    def switch_mode(self):
        if self.mode == 1:
            self.mode = 2  # go into TRUEMAP mode, ARUCO markers are fixed

            # load TRUEMAP to obtain true taglist & markers
            self.markers, self.taglist = self.parse_groundtruth("./TRUEMAP_ALIGNED.txt")
            print("Robot state: ", self.robot.state.reshape(1, -1))
            print("Markers: \n", self.markers.T)
            print("Taglist: ", self.taglist, "\n")

            # set convriance of landmarks to 0 (no uncertainty)
            self.P = np.zeros((23, 23))
            # self.P[3:,3:] = np.zeros((20, 20))

            # reset robot pos back to 0,0,0
    ...
    def predict(self, raw_drive_meas):
        ...
        self.robot.drive(raw_drive_meas)  # predict mu_k at next operate.py loop based on motion model (updates self.robot.state)
