...
cell_size = 0.1                      # (in m) changeable
skip_block = 1                        # (blocks to skip in 1 straight path) changeable
end_stop_block = 1                    # (how many blocks away from the end point to stop) changeable

num_of_cell = int(3.2/cell_size)
zero_adjust = num_of_cell/2
scale = 32/num_of_cell
...
def reconstruct_path(grid, came_from, current):
    ...
    path = []  
    for i in range(len(path_temp)-2,end_stop_block,-1):
        x_coor = round((path_temp[i][0] - zero_adjust)*cell_size,2)
        y_coor = round(-(path_temp[i][1] - zero_adjust)*cell_size,2)
...
def a_star(truemap_file, end, start = (0,0)):
    ...
    obstacle_list = []
    start_tuple = (zero_adjust - start[1]*10/scale, zero_adjust + start[0]*10/scale)
    end_tuple = (zero_adjust - end[1]*10/scale, zero_adjust + end[0]*10/scale)
    # print(start_tuple)
    # print(end_tuple)

    for pose in js.keys():
        x_coor = np.round(js[pose]['x'],1)*10
        y_coor = np.round(js[pose]['y'],1)*10
        obstacle_column_index = zero_adjust + x_coor/scale
        obstacle_row_index = zero_adjust - y_coor/scale
        if (obstacle_row_index,obstacle_column_index) != start_tuple and (obstacle_row_index,obstacle_column_index) != end_tuple:
            for i in range(-1,2):
                for j in range(-1,2):
                    obstacle_list.append((obstacle_row_index+i,obstacle_column_index+j))
    ...
    for row in range(rows):
        if (row == 0 or row == num_of_cell):
            truemap.write("1 " * (num_of_cell + 1))
            truemap.write("\n")
        else:
            for column in range(cols):
                if column == 0 or column == num_of_cell:
                    truemap.write("1 ")
                else:
                    if (row,column) == end_tuple:
                        truemap.write("3 ")
                    elif (row,column) == start_tuple:
                        truemap.write("2 ")
                    elif (row, column) in obstacle_list:
                        truemap.write("1 ")
                    else:
                        truemap.write("0 ")
            truemap.write("\n")
...
if __name__ == "__main__":
    path = a_star('TRUEMAP_ALIGNED.txt', (-0.8 ,-0.8), (0.0,0.0))
    print(path)
