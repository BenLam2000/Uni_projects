...
class AutoFruitSearch:
    def __init__(self, pibot, scale, baseline, baseline_left, baseline_right, drive_ticks, turn_left_ticks, turn_right_ticks, turn360_angle, turn360_delay, detector=None):
        ...
        self.turn360_delay = turn360_delay
    ...
    def drive_to_point(self, waypoint):
        ...
        self.turn(np.rad2deg(desired_heading))
    ...
    def turn_combine(self, angle_deg):
        if abs(angle_deg) > self.basic_angle:
            sections = int(abs(angle_deg)/self.basic_angle)
    ...
    def turn(self, angle_deg, command1=None, return_turn_time_lv_rv=False):
        ...
        print(f"Turning {angle_deg} degrees")
        # input("")
        lv, rv = 0, 0
        if command1 is None:
            if motion == [0, 1]:  # turn left
                lv, rv = self.pibot.set_velocity(
                    [0, 1.2], turning_tick=self.turn_left_ticks, tick=self.drive_ticks, time=turn_time)
            elif motion == [0, -1]:  # turn right
                lv, rv = self.pibot.set_velocity(
                    [0, -1.2], turning_tick=self.turn_right_ticks, tick=self.drive_ticks, time=turn_time)
        else:
            if motion == [0, 1]:  # turn left
                lv, rv = self.pibot.set_velocity(
                    [0, command1], turning_tick=self.turn_left_ticks, tick=self.drive_ticks, time=turn_time)
            elif motion == [0, -1]:  # turn right
                lv, rv = self.pibot.set_velocity(
                    [0, -command1], turning_tick=self.turn_right_ticks, tick=self.drive_ticks, time=turn_time)
    ...
    def drive(self, dist, command1=None):
        drive_time = abs(dist) / (self.drive_ticks * self.scale)  # replace with your calculation
        direction = math.copysign(1, dist)
        motion = [direction, 0]
        # print("Driving for {:.2f} seconds".format(drive_time))
        print(f"Driving for {dist} m")
        # input("")
        if command1 is None:
            if motion == [1,0]:
                self.pibot.set_velocity([0.98, 0], tick=self.drive_ticks, time=drive_time)
            elif motion == [-1,0]:
                self.pibot.set_velocity([-0.88, 0], tick=self.drive_ticks, time=drive_time)
        else:
            if motion == [1,0]:
                self.pibot.set_velocity([command1, 0], tick=self.drive_ticks, time=drive_time)
            elif motion == [-1,0]:
                self.pibot.set_velocity([-command1, 0], tick=self.drive_ticks, time=drive_time)
    ...
    @staticmethod
    def generate_path(map_, goal, start):
