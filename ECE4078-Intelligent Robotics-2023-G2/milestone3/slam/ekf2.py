...
class EKF2:
    ...
    def __init__(self, robot=None):
        ...
        # load TRUEMAP to obtain true taglist & markers
        self.true_markers, self.true_taglist = self.parse_groundtruth("./TRUEMAP_ALIGNED.txt")

        # initialize normally first
        self.markers = self.true_markers
        self.taglist = self.true_taglist
    ...
    def parse_groundtruth(self, fname):
        with open(fname, 'r') as f:
            ...
            for key in gt_dict:
                if key.startswith("aruco"):
                    ...
                    keys.append(aruco_num)
                    point = np.reshape([gt_dict[key]["x"], gt_dict[key]["y"]], (2, 1))
                    points.append(point)

        return np.hstack(points), keys
    ...
    def predict(self, raw_drive_meas):
        ...
        self.robot.drive(
            raw_drive_meas)  # predict mu_k at next operate.py loop based on motion model (updates self.robot.state)
        F = self.state_transition(raw_drive_meas)  # A
        Q = self.predict_covariance(raw_drive_meas)  # sigma_Q
        # x = self.get_state_vector()  # combine state with markers ## WHAT'S IT FOR??

        # TODO: add your codes here to compute the predicted x
        self.P = F @ self.P @ F.T + Q  # update robots uncertainty

    def robot_to_world_frame(self, robot_frame):
        th = self.robot.state[2]
        robot_xy = self.robot.state[0:2, :]
        R_theta = np.block([[np.cos(th), -np.sin(th)], [np.sin(th), np.cos(th)]])
        world_frame = robot_xy + R_theta @ robot_frame
        return world_frame

    # the update step of EKF
    def update(self, measurements):  # pass ground truth lms into here
        if not measurements:
            return
        ...
        # Stack measurements and set covariance (ACTUAL MEASURED MARKERS, NOT TRUEMAP)
        measurements_pos_robot = [lm.position.reshape(-1, 1) for lm in measurements]
        # measurements_pos_world = [self.robot_to_world_frame(measurements_pos_robot) for lm in measurements_pos_robot]
        z = np.concatenate(measurements_pos_robot, axis=0)
        R = np.zeros((2 * len(measurements), 2 * len(measurements)))
        for i in range(len(measurements)):
            R[2 * i:2 * i + 2, 2 * i:2 * i + 2] = 0.0*np.ones(2)
            # R[2 * i:2 * i + 2, 2 * i:2 * i + 2] = measurements[i].covariance

        # Compute own measurements (TRUEMAP)
        z_hat = self.robot.measure(self.markers, idx_list)
        z_hat = z_hat.reshape((-1, 1), order="F")
        H = self.robot.derivative_measure(self.markers, idx_list)

        x = self.get_state_vector()
        K = self.P @ H.T @ np.linalg.inv(H @ self.P @ H.T + R)  # Find kalman gain
    ...
    def add_landmarks(self, measurements):
        ...
        for lm in measurements:
            ...
            # lm_bff = lm.position
            # lm_inertial = robot_xy + R_theta @ lm_bff  # transform robot->world frame

            lm_tag = int(lm.tag)
            self.taglist.append(lm_tag)
            true_tag_index = self.true_taglist.index(lm_tag)
            true_marker = self.true_markers[true_tag_index*2:true_tag_index*2+2]
            self.markers = np.concatenate((self.markers, true_marker), axis=1)
...
if __name__ == "__main__":
    # utils = MappingUtils()
    # utils.load("../lab_output/slam.txt")
    # print(utils.markers.shape)
    # print(utils.taglist)
    ekf2 = EKF2()

    markers, taglist = ekf2.parse_groundtruth("../TRUEMAP_ALIGNED.txt")
    print(markers.shape)
    print(taglist)
